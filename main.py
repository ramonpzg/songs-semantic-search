...
def get_vectors():
    ...
    get_index = metadata.loc[metadata["name"] == song, "index"].iloc[
        0
    ]  # get the index of such a song
            ...
            genre_filter = models.Filter(
                must=[
                    models.FieldCondition(
                        key="genre", match=models.MatchValue(value=filters.value)
                    )
                ]
            )
            music = client.search(
                collection_name=collection,
                query_vector=song_vector[0].vector,
                query_filter=genre_filter,
                limit=num_songs.value,
            )[1:]
        else:
            music = client.search(
                collection_name=collection,
...
with ui.label("How many songs would you like to get back? 🤔").classes(
    "w-200 self-center mt-10"
):
    num_songs = ui.slider(min=1, max=30, step=1, value=11)
    ui.linear_progress().bind_value_from(num_songs, "value")

with ui.label("Filters you can apply to your search 🔎").classes(
    "w-200 self-center mt-5"
):
    filters = ui.radio([None] + metadata.genre.unique().tolist(), value=None).props(
        "inline color=#ab003c"
    )
...
song_selection = (
    ...
    .style("width: 700px")
    .classes("w-200 self-center mt-10 transition-all")
)
...
ui.run(
    title="Vector Databases for Music Search",
